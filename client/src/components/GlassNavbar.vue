...
const navLinks=[{label:'赛事市场',path:'/'},{label:'票务大厅',path:'/tickets'},{label:'赛季徽章',path:'/badges'},{label:'服务兑换',path:'/services'},{label:'A / B 钱包',path:'/wallet'},{label:'我的',path:'/me'}];
...
@media(max-width:620px){.nav-right span{display:none}.brand :deep(.logo){font-size:18px!important}.nav-link{font-size:13px}.nav-links:not(.nav-links--staff){display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:0;overflow:visible}.nav-links:not(.nav-links--staff) .nav-link{font-size:12px;text-align:center}}
</style>
