...
async function claim() {
  ...
  ElMessage.success('待成熟收益已生成，7 天后方可领取');
}
</script>
    ...
    <div class="detail-grid">
      <section class="detail-column">
        <GlassCard padding="24px" :hoverable="false">
          <div class="card-heading"><div><p>按比例分配</p><h2>奖金池状态</h2></div><span>{{ bucketLabel(market.stakeBucket) }}</span></div>
        ...
        <GlassCard padding="24px" :hoverable="false">
          <div class="card-heading"><div><p>结算安全</p><h2>结算与纠错路径</h2></div></div>
      ...
      <aside>
        <GlassCard v-if="auth.user?.role === 'student'" class="position-panel" padding="24px" :hoverable="false">
          <p class="panel-kicker">私有仓位</p><h2>建立预测仓位</h2>
        ...
        <GlassCard v-else class="position-panel" padding="24px" :hoverable="false"><p class="panel-kicker">角色视图</p><h2>只读市场视图</h2><p class="fine-print">当前证书角色为{{ roleLabel(auth.user?.role) }}。只有学生身份可以建立仓位或提出参与者挑战；管理动作请前往运营控制台。</p><button class="soft-btn" @click="router.push('/admin')">前往运营控制台</button></GlassCard>
        ...
        <GlassCard v-if="market.status === 'PROVISIONAL_FINALIZED' && auth.user?.role === 'student'" class="challenge-panel" padding="24px" :hoverable="false">
          <h3>领取待成熟收益</h3><p>当前结算轮次 {{ market.settlementEpoch }}。收益会先进入 7 天等待期。</p><button class="soft-btn" @click="claim">生成待成熟收益</button>
        </GlassCard>
      </aside>
    </div>
  </div>
...
.activity-meta { display: flex; flex-wrap: wrap; gap: 14px; color:var(--ec-muted); margin: 17px 0;font:500 10px var(--ec-font-mono) }.timeline { display: grid; gap: 11px; margin-top: 18px; }.timeline div { display: grid; grid-template-columns: 28px 1fr; align-items: center; gap: 10px; }.timeline b { display: grid; place-items: center; width: 26px; height: 26px; border-radius: 50%; background:rgba(0,113,227,.1); color:var(--ec-red);font-family:var(--ec-font-mono) }.timeline span { color:var(--ec-ink-2); font-size: 13px; }
.position-panel{background:var(--ec-card-white);border-color:var(--ec-line)}.position-panel h2 { font-size: 24px; margin-top: 4px; }.available { color:var(--ec-muted); margin: 8px 0 18px; }.outcome-buttons { display: grid; gap: 8px; }.outcome-buttons button { display: flex; justify-content: space-between; border: 1px solid var(--ec-line); border-radius: 11px; padding: 12px; background:transparent; cursor: pointer; color: inherit; }.outcome-buttons button.selected { border-color:var(--ec-red);background:rgba(0,113,227,.08)}.position-panel label { display: block; margin: 18px 0 8px;font:500 10px var(--ec-font-mono);letter-spacing:.1em}.position-panel :deep(.el-input-number) { width: 100%; }.submit-btn,.soft-btn,.danger-btn { width: 100%; border: 0; border-radius: 11px; padding: 12px;font:700 15px var(--ec-font-display);cursor: pointer; margin-top: 14px; }.submit-btn { color:#fff;background:var(--ec-red)}.submit-btn:disabled { opacity:.45; cursor:not-allowed; }.soft-btn { color:#fff;background:var(--ec-red)}.danger-btn { color:var(--ec-danger);background:rgba(196,59,59,.08)}.loading-state { text-align:center;padding:100px;color:var(--ec-faint); }
