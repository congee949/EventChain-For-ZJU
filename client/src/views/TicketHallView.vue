...
import { activityDisplayStatusLabel, applicationStatusLabel, ticketStatusLabel } from '../utils/display.js';
...
<template>
  <div class="ticket-hall">
    <section class="ticket-hero"><div><p>承诺—揭示抽签</p><h1>活动票务大厅</h1><span>报名不是先到先得。验证者预先承诺种子，截止后由组织者揭示并确定中签名单。</span></div><div class="hero-count"><strong>{{ activeActivities.length }}</strong><small>未结束活动</small></div></section>

    <section class="section">
      <div class="section-title"><div><p>活动进度</p><h2>可参与的活动</h2></div><span>申请记录仅本人可见</span></div>
      <div class="ticket-events-grid">
        <GlassCard v-for="item in activeActivities" :key="item.id" class="ticket-event-card">
          <div class="event-card-main">
            <div class="te-header"><span class="category-pill">{{ categoryName(item.categoryId) }}</span><span class="status-pill" :class="{ 'status-pill--waiting': item.status === 'APPLICATION_OPEN' && new Date(item.applicationCloseAt).getTime() <= clock }">{{ activityDisplayStatusLabel(item, clock) }}</span></div>
            <h3>{{ item.title }}</h3>
...
.status-pill--waiting{color:var(--ec-danger);background:rgba(196,59,59,.08)}
