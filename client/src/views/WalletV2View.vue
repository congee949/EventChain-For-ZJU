...
<template>
  <div class="v2-page">
    <header class="v2-hero"><p class="eyebrow">PRIVATE WALLET</p><h1>A / B 钱包</h1><p>A 是封闭积分；B_paid 有 A 储备并可兑回，B_bonus 来自签到/补偿且不可兑回。</p></header>
    ...
    <GlassCard :hoverable="false">
      ...
      <div v-if="auth.user?.role === 'student'" class="v2-actions wrap">
        ...
        <el-input v-model="state.toAccountId" placeholder="收款 opaque account ID" />
        <el-button @click="act(() => finance.transfer(state.categoryId, state.toAccountId, state.amount), '转让已提交')">转让 B_paid</el-button>
      </div>
      <p v-else class="muted">当前角色为 {{ auth.user?.role }}；兑换、兑回和转让仅对学生积分账户开放。</p>
